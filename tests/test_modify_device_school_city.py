...
def test_modify_device_info():
    ...
    json_file = open(
        os.path.join(file_path, "json_data", "test_modify_device_school_city.json"),
        encoding="utf-8",
    )
    ...
    result = requests.request("PUT", url, headers=headers, data=data.encode())

    assert result.status_code == 201


if __name__ == "__main__":
    test_modify_device_info()
